...
class TypingInterface(QWidget):
    ...
    def setup_ui(self):
        ...
        controls_and_stats_group = self.create_combined_controls_and_stats()
        layout.addWidget(controls_and_stats_group)
    ...
    def setup_connections(self):
        ...
        # No auto-stop - sessions only end manually
    ...
    def stop_session(self):
        ...
        if self.speed_engine.is_session_active():
            # End the session manually and handle the result
            score_record = self.speed_engine.end_session()
            # Handle the session end manually (no automatic timer callback)
            self.handle_manual_session_end(score_record)
    ...
    def handle_manual_session_end(self, score_record):
        """Handle manual session end (only when user clicks Stop)"""
        # Reset UI
        self.typing_input.setEnabled(False)
        ...
        if score_record:
            ...
            # Use QTimer to ensure dialogs are shown in the main thread
            from PyQt6.QtCore import QTimer
            QTimer.singleShot(100, lambda: self._show_session_dialogs(score_record))
        
        self.current_session = None
        self.session_ended.emit(score_record)
    
    def _show_session_dialogs(self, score_record):
        """Show session dialogs with proper error handling"""
        try:
    ...
    def show_leaderboard(self):
        ...
        top_scores = self.speed_engine.get_leaderboard(limit=50)  # Show more scores
        
        if top_scores:
            # Create table
            table = QTableWidget(len(top_scores), 6)  # Added date column
            table.setHorizontalHeaderLabels(["Rank", "Name", "WPM", "Accuracy", "Mode", "Date"])
            ...
            for i, score in enumerate(top_scores):
                ...
                table.setItem(i, 5, QTableWidgetItem(score.date.strftime("%m/%d %H:%M")))
    ...
    def update_display(self):
        ...
        if self.speed_engine.is_session_active():
            ...
            elapsed = self.speed_engine.get_elapsed_time()
